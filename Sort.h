...
T** Sort<T>::quickSort(T** items, int numItems, int (*compare) (T* one, T* two))
{
   ...
   //this is in case the original, unsorted array is also needed
	...
	Sort::_quickSort(array, first, last, compare);
	return array;
...
void Sort<T>::_quickSort(T** items, int first, int last, int (*compare) (T* one, T* two))
{
   ...
   if (first < last)
   {
		...
		pivotIndex = Sort::partition(items, first, last, compare);
		Sort::_quickSort(items, first, pivotIndex-1, compare);
		Sort::_quickSort(items, pivotIndex+1, last, compare);
...
int Sort<T>::partition(T** items, int first, int last, int (*compare) (T* one, T* two))
{
   ...
   choosePivot(items, first, last); 
	...
	{
		if((*compare)(items[i], items[pivot_1])<0)
		{
			pivot_2++;
			temp = items[i];
			items[i] = items[pivot_2];
			items[pivot_2] = temp;
		}
		
		temp = items[pivot_1];
		items[pivot_1] = items[pivot_2];
		items[pivot_2] = temp;
		return pivot_2;
	}
}
